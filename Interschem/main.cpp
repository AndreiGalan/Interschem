...
void desenare1_blocuri()//Andrei si Cosmin
{
    ...
    while (!nu_este_oprita_desenarea1(xMouse, yMouse))
    {
        ...
        else if (este_selectat_bloculDECIZIE1(xMouse, yMouse) && ok == 1)
        {
            ...
            interogation.xPos = 1360;
            interogation.yPos = 760;
            scan_window_decizie(arr, point, 25, interogation);
            if (strlen(arr) >= 9)
...
void viziteaza_cod(int a[], int blocc, FILE* fisier3, int maxi)//Cosmin
{
    int nr = 0;
    for (int i = 1; i <= nrLegaturi; i++)
    {
        ...
        if (vecLegaturi[i].nod1 == blocc)
        {
            ...
            {
                if (a[nod1] ==maxi && maxi>2 && !nr)
                {
                    fprintf(fisier3, "while%s {\n", information + 2);
                    nr++;
                    a[nod1] = maxi+1;
                    viziteaza_cod(a, vecLegaturi[i].nod2, fisier3,maxi);
                    fprintf(fisier3, "\n}\n");


                }
                else if (a[nod1] ==maxi+1 && nr != 0)
                {
                    fprintf(fisier3, "\n");
                    viziteaza_cod(a, vecLegaturi[i].nod2, fisier3,maxi);
                }
               else  if (((a[nod1] < maxi && maxi>=2)||(a[nod1]==2))  && !nr)
                {
                    fprintf(fisier3, "%s {\n", information);
                    nr++;
                    viziteaza_cod(a, vecLegaturi[i].nod2, fisier3,maxi);
              ...
              else   if (((a[nod1] <maxi && maxi>=2)||(a[nod1]==2)) && nr != 0)
                {
            ...
            if (vecNoduri[vecLegaturi[i].nod2].tip != 'T' && a[vecLegaturi[i].nod2] != maxi+1 && vecNoduri[nod1].tip != 'D')
                viziteaza_cod(a, vecLegaturi[i].nod2, fisier3,maxi);
        }
...
void salvare_cod_CPP(char nume_fisier[])//Cosmin
{
    ...
    for( int i=1;i<index; i++)
        if(vizitat[i]>maxi)
            maxi=vizitat[i];
    viziteaza_cod(vizitat, bloc_curent, fisier3, maxi);
...
void menu()//Cosmin
{
    ...
    {
            myOuttextXY(1360, 700, culbk, "Inca nu a fost ");
            myOuttextXY(1360, 720, culbk, "creata schema ");
        ...
        else if (este_selectat_salvare_schema(mouse_x, mouse_y) && ok_schema_creata)
        {
          ...
          {myOuttextXY(1360, 700, 4, "Inca nu a fost ");
            myOuttextXY(1360, 720, 4, "creata schema ");
          }
